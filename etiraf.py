  ...
  if event.is_private:
    async for usr in client.iter_participants(event.chat_id):
     ...
     return await event.reply(f"{ad} {startmesaj}", buttons=(
                      ...
                      [Button.url('📜 İtiraf Kanalı', f'https://t.me/tritiraf')],
                      [Button.url('📣 Destek', f'https://t.me/goldsohbettr'),
                       Button.url('👨🏻‍💻 Sahibim', f'https://t.me/cavresoo')]
                    ),
                    link_preview=False)
...
async def anonim(event):
    ...
    async for usr in client.iter_participants(event.chat_id):
     ...
     tesdiq = await yeni_etiraf.reply("İtirafın Onaylanması ?", buttons=(
                      [
                       Button.inline("✅ Onayla", data="tespit"
                       ),
...
async def sil(event):
    ...
    if not tesdiq.is_reply:
      return await tesdiq.edit("Silme sırasında hata oluştu Tekral Deneyiniz.
      ")
    if tesdiq.is_reply:
      etiraf = await tesdiq.get_reply_message()
      await etiraf.delete()
      await event.edit("🗑f"ititraf Silindi")
